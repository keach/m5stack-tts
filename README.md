...
Ambientには、`d1`から順に気温、湿度、気圧、直近1時間雨量、高温アラート閾値、現在の降雨状態、Wi-Fi RSSIを送信します。すべての送信でOpenWeather APIから正常に気象情報を取得した時刻を `created` として指定し、画面下部にも同じ時刻を `yyyy.mm.dd. hh:mm` 形式でAmbient送信結果とともに表示します。時刻が同期されていない場合はAmbient送信を行わず、SDカードへの記録は継続します。
...
予報画面で1分間ボタン操作がない場合は、現在天気画面へ自動的に戻ります。予報の取得中、取得失敗、未取得も画面に表示し、更新に失敗した場合は直前に取得できた予報を保持します。
