...
Wi-Fi・NTPの初期化後、P2P地震情報 JSON API v2のWebSocketへ接続します。緊急地震速報（コード556）は対象地域にかかわらず表示し、設定した都道府県が予報区に含まれる本番情報だけ、同一イベントにつき1回専用警告音を鳴らします。対象都道府県を含む地震情報（コード551）は最大震度、震源、マグニチュード、発生時刻を60秒間表示します。表示優先度は緊急地震速報、地震情報、気象アラート、通常画面の順です。

対象地域の情報は画面スリープ中でもLCDを復帰させます。緊急地震速報は最終更新から2分間、取消報は30秒間表示し、その後は元の画面へ戻ります。受信中も天気取得、SDログ、Ambient・ThingSpeak送信は継続します。音声読み上げと地震履歴の保存・閲覧は後続Issueで対応します。

動作試験では `EARTHQUAKE_USE_SANDBOX` を `true` にするとP2P地震情報のサンドボックスへ接続します。サンドボックス由来の情報は画面に試験表示し、通常の警告音を鳴らしません。試験後は必ず `false` に戻してください。
