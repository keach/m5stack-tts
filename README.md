...
次は降雨アラートを追加する予定です。現在値の直近1時間雨量に加え、OpenWeatherの予報データに含まれる降水確率と予想降水量を使って「雨が降りそう」な状態を判定します。判定閾値、再通知条件、夜間の音声通知ルールは未決定です。ログは高温アラートと分離し、`/rain_alerts.csv` などの専用ファイルへ記録します。
